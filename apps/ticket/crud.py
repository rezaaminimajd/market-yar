...
def get_ticket(db: Session, user_id: int):
    ...
    elif user.user_type == account_models.UserType.ADMIN:
        return get_addmin_tickets(db, user_id)
    elif user.user_type == account_models.UserType.BOSS:
        get_bos_tickets(db, user_id)

def get_bos_tickets(db, user_id: int):
    all_tickets = []
    all_tickets += get_user_tickets(db, user_id)
    all_tickets += db.query(models.Ticket).filter(models.Ticket.creator_type == account_models.UserType.ADMIN and models.Ticket.parent_ticket_id == null()).all()
    return all_tickets
...
def answer(db: Session, answer: schemas.Answer_ticket, user_id: int):
    user = db.query(account_models.User).filter(account_models.User.id == user_id).one()
    ticket = models.Ticket(message = answer.message, creator_id = user_id, parent_ticket_id = answer.parent_ticket_id, creator_type = user.user_type)
    db.add(ticket)
    db.commit()
    return "done"
...
def set_status(db: Session, new_status: schemas.New_status, user_id: int):
    ...
    for ticket in tickets:
        if ticket.id == new_status.ticket_id:
            db.query(models.Ticket).filter(models.Ticket.id == new_status.ticket_id).update({models.Ticket.status: new_status.status})
            db.commit()
            return "done"
